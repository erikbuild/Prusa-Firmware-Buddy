...
public:
    ...
    static Result exec(const string_view_utf8 &prompt, float initial_value, const Config &config);

    inline Result result() const;

protected:
    DialogNumericInput(const string_view_utf8 &prompt, float initial_value, const Config &config);
