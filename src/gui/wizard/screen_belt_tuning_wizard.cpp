...
public:
    ...
    void update(const fsm::PhaseData &data_) {
        ...
        if (data.encoded_frequency) {
            ...
            {
                const size_t end = std::clamp(static_cast<int>(std::ceil(data.progress_0_255 / 255.0f * ScreenBeltTuningWizard::graph_width)), 0, ScreenBeltTuningWizard::graph_width);
                ...
                for (size_t i = start; i < end; i++) {
                    screen.graph_data[i] = std::max(screen.graph_data[i], static_cast<uint8_t>(std::clamp(static_cast<int>(start_val + (end_val - start_val) * (i - start) / (end - start)), 0, ScreenBeltTuningWizard::graph_height - 1)));
                }
