...
public:
    ...
    void init(can::cyphal::PortList &port_list, auto &registers) {
        port_list.add(event_sender);
        port_list.add(accept_event_server);

        event_sender.add_to_task();
        accept_event_server.add_to_task();

        registers.add_port_name_set("pub.event", prusa3d_nfc_event_Event_1_0_FULL_NAME_AND_VERSION_, event_sender.get_port_id());
        registers.add_port_name_set("srv.accept_event", prusa3d_nfc_command_AcceptEvent_1_0_FULL_NAME_AND_VERSION_, accept_event_server.get_port_id());
    }
